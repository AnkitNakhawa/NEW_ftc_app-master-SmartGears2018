...
@Autonomous(name = "ColorOpTest_Ankit", group = "pushbot")
//@disabled
public class ColorSensorOpModeTest1_Ankit extends LinearOpMode {
...
//            telemetry.update();
//        }
        if (hsvValues[0]>=20 && hsvValues[0]<=28 && distanceSensor.getDistance(DistanceUnit.CM)<= 5){
            telemetry.addData("The Color Is ", "Yellow");
            telemetry.update();

        }
        if (hsvValues[0]>=29 && hsvValues[0]<=50 && distanceSensor.getDistance(DistanceUnit.CM)<= 5){

            telemetry.addData("The Color Is ", "White");
            telemetry.update();
        }
        if (distanceSensor.getDistance(DistanceUnit.CM)>= 5){
            telemetry.addData("The Color Is", "Null");
            telemetry.update();
        }
